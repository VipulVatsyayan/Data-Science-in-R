...
# Making the values in numeric. K for 1000 and M for 1000000 
...
# Checking for the player overall rating

fifa_data %>% 
  ggplot(aes(x= Overall))+ geom_histogram(binwidth = 2)

# This is normally distributed with a mean arond 66


# Checking for rating vs age
fifa_data %>% 
  filter(!PositionGroup %in% c("GK", "Unknown")) %>%
  group_by(Age) %>% 
  summarise(Rating = mean(Overall)) %>% 
  ggplot(aes(x= Age, y =Rating))+ geom_line()+ ggtitle("Mean Rating vs Age")


# Checking for rating for differnt position vs age

fifa_data %>% 
  filter(!PositionGroup %in% c("GK", "Unknown")) %>%
  group_by(PositionGroup ,Age) %>% 
  summarise(Rating = mean(Overall)) %>% 
  ggplot(aes(x= Age, y =Rating))+ geom_line()+ ggtitle("Mean Rating vs Age")+ facet_wrap(~PositionGroup)
...
p1<- fifa_data %>% 
  ggplot(aes(x= PositionGroup, y= ValueNumeric_pounds))+ geom_boxplot() +scale_y_log10()
...
p2<-  fifa_data %>% 
  ...
  scale_y_log10()+
  coord_flip()+
  facet_wrap(~PositionGroup, scales = "free")  # Scales = free removes the ones (Eg RB, ST, LW etc) where the boc plot is unavailable 
...
cor(fifa_data$Overall, fifa_data$ValueNumeric_pounds, method = "spearman")   #  We use spearman becaust the relationship inot linear as seen in the graph

# We can say there is high correlation
...
spearman_cor_with_overall <- fifa_data %>%
  ...
  select(-gk_vars, -ID, - `Jersey Number`, -ValueMultiplier) %>% 
  as.matrix() %>%
  na.omit() %>%
  cor(method = "spearman") 
...
# 
# The below correlation matrix data displays the correlations between the Overall rating and other key attribute variables for all players except goalkeepers.
#Both the Pearson and Spearman correlation methods were used to display the differences.
# The Spearman correlation method is more robust with dealing with extreme outliers, hence the player Value having the highest spearman correlation (0.92). 
#Reactions, Composure, Special and (surprisingly) Wage rounding out the top 5 correlated variables with Overall rating using the Spearman method.
#I say surprisingly because Soccernomics (Kuper.S, Szymanski.S, 2014) stated the best predictor of team success was the teams wage bill.
# Using the Pearson method, we get different results. the top 5 correlated variables with the Overall rating is Reactions, Composure, Special, ShortPassing and BallControl. 
#Both value and wage don’t appear ni this list because they are prone to large outliers.
